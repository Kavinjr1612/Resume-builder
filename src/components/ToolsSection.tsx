...
const ToolsSection: React.FC = () => {
  ...
  return (
    <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-gray-50 dark:from-slate-950 to-blue-50 dark:to-slate-900">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-lg sm:text-xl text-gray-600 dark:text-slate-400 max-w-3xl mx-auto mb-4 px-4">
            Comprehensive suite of tools to accelerate your job search and career growth
          </p>
          <div className="inline-flex items-center space-x-2 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300 px-3 py-2 sm:px-4 sm:py-2 rounded-xl text-xs sm:text-sm font-medium">
            ...
            <button
              ...
              className={`px-3 py-2 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-colors border shadow-sm
                ${activeCategory === category 
                  ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white border-blue-600' 
                  : 'bg-white dark:bg-slate-900 dark:bg-slate-950 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-800 hover:bg-blue-50'}`}
            >
              {category}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
          {filteredTools.map((tool) => (
            <div key={tool.id} className="bg-white dark:bg-slate-900 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300 relative overflow-hidden group">
              {/* Tool Icon and Info */}
              <div className="flex items-start space-x-3 sm:space-x-4 mb-3 sm:mb-4">
                <div className="bg-gradient-to-br from-blue-100 to-blue-200 p-2 sm:p-3 rounded-xl text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
                  {tool.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-1 line-clamp-2">{tool.title}</h3>
                  <span className="text-xs sm:text-sm text-indigo-600 dark:text-indigo-400 font-medium bg-blue-50 px-2 py-1 rounded-full">
                    {tool.category}
                  </span>
                </div>
              </div>
              
              <p className="text-gray-600 dark:text-slate-400 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base line-clamp-3">{tool.description}</p>
              
              <div className="mt-auto pt-4">
                <button 
                  className="w-full flex items-center justify-center space-x-2 px-3 py-2 sm:px-4 sm:py-3 rounded-lg font-medium transition-all duration-300 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-600 dark:text-slate-400 cursor-not-allowed relative overflow-hidden text-sm sm:text-base"
                  disabled
                >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-teal-600 rounded-2xl p-6 sm:p-8 text-center text-white mb-6 sm:mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
            <div className="bg-white dark:bg-slate-900 dark:bg-slate-950/20 backdrop-blur-sm rounded-lg px-4 py-2 sm:px-6 sm:py-3 text-white">
